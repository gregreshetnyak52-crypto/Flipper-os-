...
- **Flipper OS Toolkit v1.2** входит в прошивку (`apps/Tools/flipper_os.fap`).
...
| **Settings** | Сброс сохранённых данных Toolkit: рекорд «Змейки», счётчик или всё сразу | ↑/↓ — пункт, OK — подтвердить (дважды), ← — отменить подтверждение |
| **About** | Версия Toolkit, происхождение прошивки, ветка/коммит и дата сборки Unleashed, модель устройства | — |

Рекорд в «Змейке», значение счётчика, выбор костей, настройки Морзе, фонарика
и таймера сохраняются на SD-карте в `apps_data/flipper_os/settings.dat`. Внизу
меню — **Settings** (сброс сохранённых данных Toolkit) и **About** (версия
Toolkit, ветка/коммит и происхождение прошивки, дата сборки).
